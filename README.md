...
An automated utility design tool — hard-forked from [GeoLibre](https://github.com/opengeos/GeoLibre) (MIT licensed) as its GIS/mapping foundation. AU DNA Engine adds automated network-design capabilities on top: draw a project area, answer a few AI-guided questions about the utility (water, sewer, stormwater, electric, or fiber), and get an auto-generated junction-and-line network as a starting design.

This repo carries GeoLibre's full history and codebase; everything below this section is GeoLibre's own documentation, kept intact since most of the app (the map workspace, data I/O, styling, processing tools, plugin system) is unchanged. Utility-design-specific features are additive, documented separately as they land.
...
- Add Data menu for XYZ tiles, WMS and WFS (with layers and feature types discovered from the service's GetCapabilities so you pick from a populated dropdown), GeoJSON URLs, GeoRSS feeds (URL or file), vector tiles, COG and GeoTIFF rasters, Cloud-Optimized NetCDF/HDF (via kerchunk references), MBTiles, ArcGIS FeatureServer and VectorTileServer layers, PMTiles, Zarr, LiDAR, 3D Tiles (including authenticated tilesets via custom request headers), CAD drawings (DXF/DWG) with a drawing-layer picker and CRS selector, Gaussian splats, glTF/GLB 3D models placed at coordinates, georeferenced video overlays, and geotagged photos imported as a point layer from their EXIF GPS (with manual placement and drag for photos lacking coordinates), with a fully internationalized dialog, comma decimal support, drag-and-dropped CSV coordinate files, sample-data dropdowns on every upstream-backed panel for loading ready-made example datasets, and a saved service library for storing and re-adding frequently used web-service endpoints
...
MIT — this fork is (c) 2026 RyanHildebrandGIS, building on GeoLibre (c) 2026 Qiusheng Wu. See [LICENSE](LICENSE).
